...
/*
 * @(#)HttpRequest.java	1.22 03/08/06
 */

/*
 * @(#)HttpRequest.java	1.10 08/20/02
 */
 ...
 */
    ...
    public void setContent(String content) {
        if (_method instanceof PostMethod) {
            ((PostMethod) _method).setRequestBody(content);
        } else if (_method instanceof PutMethod) {
            ((PutMethod) _method).setRequestBody(content);
        }
    ...
    public void setAuthenticationCredentials(String username, String password,
        ...
        getState().setCredentials(realm, _host, cred);
    ...
    public HttpResponse execute() throws IOException, HttpException {
        ...
        HttpResponse res = new HttpResponse(_host,
            _port,
            _isSecure,
            _method,
            getState());
    ...
    public String toString() {
        ...
        if (_headers != null && _headers.length != 0) {
            ...
            for (int i = 0; i < _headers.length; i++) {
                ...
                sb.append(_headers[i].toExternalForm()).append("\n");
            }
        }
 ...
 */
    ...
    private void createCookie(String cookieHeader) {
        ...
        if (cookieLine.indexOf("$Version") == -1) {
            cookie.setVersion(0);
            _state.setCookiePolicy(CookiePolicy.NETSCAPE_DRAFT);
        }
        ...
        // HACK:  This is for our logging purposes.  This will get removed
        // by HttpClient before it gets sent and will use the Cookie we added
        // to the HttpState above instead.
        _method.addRequestHeader("Cookie", cookie.toString());
    }
    ...
    private void setBasicAuthorizationHeader() {
        UsernamePasswordCredentials cred = (UsernamePasswordCredentials)
            getState().getCredentials(null, _host);
    ...
    private void setCookieHeader() {
        if (_useCookies) {
            ...
            if (cookies != null && cookies.length > 0) {
                Header cHeader = CookiePolicy.getSpecByPolicy(CookiePolicy.RFC2109).
