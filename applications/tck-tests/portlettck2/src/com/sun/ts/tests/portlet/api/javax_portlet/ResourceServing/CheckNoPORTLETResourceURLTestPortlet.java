...
public class CheckNoPORTLETResourceURLTestPortlet extends GenericPortlet {
public static String TEST_NAME = "CheckNoPORTLETResourceURLTest";
	...
	public void serveResource(ResourceRequest request, ResourceResponse response) 
		...
		try{
			...
			writer.setStatus(ResultWriter.FAIL);
			writer.addDetail("ResourceURL.PORTLET must not throw an IllegalStateException.");
			testFail = true;
		}
		...
		// Tests to create an ActionURL
		try{
			response.createActionURL();
			writer.setStatus(ResultWriter.FAIL);
			writer.addDetail("response.createActionURL() must throw an IllegalStateException.");
			testFail = true;
		}
		catch (IllegalStateException e){	
		}
		
		// Tests to create a RenderURL
		try{
			response.createRenderURL();
			writer.setStatus(ResultWriter.FAIL);
			writer.addDetail("response.createRenderURL() must throw an IllegalStateException.");
			testFail = true;
		}
		catch (IllegalStateException e){	
		}
