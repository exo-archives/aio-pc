...
public interface GroupSession {
  ...
  /**
   * Get the portlet session object which is identified with the givven
   * instanceKey from the group session. If no portlet session with that
   * instanceKey exists it depends of the implementation wether null or a newly
   * created portlet session object is returned.
   * 
   ...
   * 
   * @return True if the group session holds a protlet session with the given
   *         key; false otherwise
   */
  ...
  /**
   * Remove the portlet session object with the given key from the group
   * session. Subsequent calls of getPortletSession with the same key should
   * either return null or a newly created object.
   * 
   * @param instanceKey Key which identifies the portlet session object to be
   *          removed.
   */
