...
public class JAXBEventTransformer {
  ...
  public static Event[] getEventsToWSRP(javax.portlet.Event event) {
    return new Event[]{getEventToWSRP(event)};
  }
  
  // convert from "javax.portlet.Event" to "org.exoplatform.services.wsrp2.type.Event"
  private static Event getEventToWSRP(javax.portlet.Event event) {
  ...
  public static Event[] getEventsToWSRP(List<javax.portlet.Event> eventsList) {
    Event[] events = new Event[eventsList.size()];
    int i=0;
    for (javax.portlet.Event event : eventsList) {
      events[i++] = getEventToWSRP(event);
    }
    return events;
  }
  
  // convert from "org.exoplatform.services.wsrp2.type.Event[]" to "List<javax.portlet.Event>"
  public static List<javax.portlet.Event> getEventsFromWSRP(Event[] eventArray) {
    if (eventArray == null)
      return null;
